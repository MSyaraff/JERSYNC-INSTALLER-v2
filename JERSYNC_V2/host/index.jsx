...
function JV2_getSizeGroup(masterDoc,type,size){
    ...
    if(typeLayer){
        var direct=JV2_findDirectGroup(typeLayer,size);
        if(direct)return direct;
        var deep=JV2_findGroup(typeLayer,size);
        if(deep)return deep;
    }
    var typeGroup=JV2_findGroup(masterDoc,type);
    if(typeGroup){
        var d=JV2_findDirectGroup(typeGroup,size);
        if(d)return d;
        return JV2_findGroup(typeGroup,size);
    }
    return null;
}
...
function JV2_generate(masterPath,outputPath,jobCSV,gapMM,fixedObjects,sizeLabelOn,strokeOn,strokeWidth,strokeColor){
    try{
...
function JV2_generateSingle(masterPath,outputPath,jobCSV,maxPerRow,gapMM){
    ...
    try{
        ...
        outDoc=app.documents.add(DocumentColorSpace.CMYK,2000,2000);
        ...
        for(var i=0;i<rows.length;i++){
            var p=rows[i].split("|");
            var type=p[0],size=p[1],sleeve=p[2];
            ...
            if(tf && JV2_applyPart(af,"F",JV2_findItem(tf,"F"),jobLayer,target,cn.front,size,"FRONT",type,mark))done++;
            else missing.push(type+" "+size+" BODY_FRONT");

            if(tb && JV2_applyPart(ab,"B",JV2_findItem(tb,"B"),jobLayer,target,cn.back,size,"BACK",type,null))done++;
            ...
            if(type==="MUSV"){
                ...
                if(mlr && JV2_applyPart(alsr,"RLS",JV2_findItem(mlr,"RLS"),jobLayer,target,"SLEEVE_RIGHT_MUS",size,"RIGHT",type,null))done++;
                else missing.push(type+" "+size+" LS RIGHT");

                if(mll && JV2_applyPart(alsl,"LLS",JV2_findItem(mll,"LLS"),jobLayer,target,"SLEEVE_LEFT_MUS",size,"LEFT",type,null))done++;
                ...
                if(sleeve==="SS"||sleeve==="BOTH"){
                    ...
                    if(tr && JV2_applyPart(assr,"RSS",JV2_findItem(tr,"RSS"),jobLayer,target,"SLEEVE_SS_RIGHT",size,"RIGHT",type,null))done++;
                    else missing.push(type+" "+size+" SS RIGHT");
                    if(tl && JV2_applyPart(assl,"LSS",JV2_findItem(tl,"LSS"),jobLayer,target,"SLEEVE_SS_LEFT",size,"LEFT",type,null))done++;
                ...
                if(sleeve==="LS"||sleeve==="BOTH"){
                    ...
                    if(lr && JV2_applyPart(alsr,"RLS",JV2_findItem(lr,"RLS"),jobLayer,target,"SLEEVE_LS_RIGHT",size,"RIGHT",type,null))done++;
                    else missing.push(type+" "+size+" LS RIGHT");
                    if(ll && JV2_applyPart(alsl,"LLS",JV2_findItem(ll,"LLS"),jobLayer,target,"SLEEVE_LS_LEFT",size,"LEFT",type,null))done++;
                    else missing.push(type+" "+size+" LS LEFT");
                }else{
