...
function renderQueue(){
  ...
  ["RN","POLO","VNECK","VFLAT","KIDS","MUSV"].forEach(function(type){
    ...
    ["SS","LS","BOTH"].forEach(function(sleeve){
      ...
      items.forEach(function(it){
        ...
        chip.innerHTML=
          '<span class="queue-size-main">'+it.job.size+(it.job.tagType && it.job.tagType!==it.job.type?'<small class="queue-tag-override">TAG '+it.job.tagType+'</small>':'')+'</span>'+
...
var JV2_STANDARD_SIZES=["XS","S","M","L","XL","2XL","3XL","4XL","5XL","6XL","7XL","8XL","9XL","10XL"];
var JV2_KIDS_SIZES=["22","24","26","28","30","32","34"];

function JV2_renderSizePicker(sizes){
  var grid=document.getElementById("sizePicker");
  if(!grid)return;

  var html="";
  for(var i=0;i<sizes.length;i++){
    html+='<button type="button" class="size-chip" data-size="'+sizes[i]+'">'+sizes[i]+'</button>';
  }
...
function JV2_refreshSizePicker(){
  var type=document.getElementById("patternType").value;

  if(type==="KIDS"){
    JV2_renderSizePicker(JV2_KIDS_SIZES);
  }else{
    JV2_renderSizePicker(JV2_STANDARD_SIZES);
  }
}

function JV2_bindSizeChips(){
  document.querySelectorAll("#sizePicker .size-chip").forEach(function(b){
    b.onclick=function(){
      var size=this.getAttribute("data-size");
      var i=state.selectedSizes.indexOf(size);

      if(i>=0){
        state.selectedSizes.splice(i,1);
...
document.getElementById("resetPatternPanel").onclick=function(){
  var type=document.getElementById("patternType");
  var sleeve=document.getElementById("patternSleeve");
  var tag=document.getElementById("tagTypeOverride");

  // Reset only the PILIH POLA panel. Queue stays untouched.
  type.value="RN";
  sleeve.disabled=false;
  sleeve.value="SS";
  tag.value="";
  ...
  setStatus("Panel Pilih Pola direset.","ok");
};

document.getElementById("clearTagType").onclick=function(){
  var tag=document.getElementById("tagTypeOverride");
  tag.value="";
  try{tag.focus();}catch(e){}
};
...
document.getElementById("addPatterns").onclick=function(){
  ...
  var tagType=(document.getElementById("tagTypeOverride").value||"").replace(/^\s+|\s+$/g,"");
...
document.getElementById("generate").onclick=function(){
  ...
  state.jobs.forEach(function(j){
    jobs.push([j.type,j.size,j.sleeve,j.tagType||j.type].join("|"));
  });
  ...
  var colorMode="CMYK";
  var selectedColorMode=document.querySelector('input[name="colorMode"]:checked');
  if(selectedColorMode)colorMode=selectedColorMode.value;

  var call='JV2_generate("'+esc(state.masterPath)+'","'+esc(state.outputPath)+'","'+esc(jobs.join(";"))+'","'+gap+'","'+fixedObjects+'","'+sizeLabelOn+'","'+strokeOn+'","'+strokeWidth+'","'+esc(strokeColor)+'","'+colorMode+'")';
...
// ===== JERSYNC V2 UPDATER =====
var JERSYNC_V2_CURRENT_VERSION="2.1.0";
var JERSYNC_V2_UPDATE_URL="https://raw.githubusercontent.com/MSyaraff/JERSYNC-UPDATE-V2/main/version-v2.json";
...
function checkForJersyncV2Updates(){
  ...
  // Hard cache-bypass:
  // timestamp + random token prevents CEP / Chromium / proxy cache from
  // reusing an older version-v2.json response.
  var cacheBust=(new Date().getTime())+"_"+Math.floor(Math.random()*1000000);
  var requestUrl=JERSYNC_V2_UPDATE_URL+"?cb="+cacheBust;

  xhr.open("GET",requestUrl,true);
  xhr.timeout=12000;

  try{xhr.setRequestHeader("Cache-Control","no-cache, no-store, max-age=0");}catch(e1){}
  try{xhr.setRequestHeader("Pragma","no-cache");}catch(e2){}
  try{xhr.setRequestHeader("Expires","0");}catch(e3){}
  ...
  xhr.onreadystatechange=function(){
    ...
    if(xhr.status>=200&&xhr.status<300){
      try{
        ...
        // Keep the actual fetched version available for UI/debugging.
        window.JERSYNC_V2_LAST_ONLINE_VERSION=latest;
        window.JERSYNC_V2_LAST_UPDATE_URL=requestUrl;
